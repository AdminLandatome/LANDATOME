...
function lire_reponse(){
	...
	if (reponse == liste_anglais[x]) {bravo();
	++ point;} else{alert(`Non, La bonne réponse était  ${liste_anglais[x]}`);};
	jeu();
...
function jeu(){
	...
	++ nb_question ;
	document.getElementById('question').innerHTML =`<div class="element"> ${liste_francais[x]} </div><br/>
	 <div class="numero">
	 <label for="name">Ecrivez ce mot en anglais :</label> 
