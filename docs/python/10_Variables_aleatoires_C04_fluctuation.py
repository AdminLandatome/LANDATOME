...
def fluctuation(n,p,nbechan):
    ...
    listefrequences = []
    for _ in range(nbechan):
        listefrequences.append(frequence_des_uns(un_echantillon(n,p)))
...
def afficheur(listefrequences,p,normalise=0,mini=0,maxi=1,s1=0,s2=0,s3=0):
    ...
    plt.plot(list(range(1,len(listefrequences)+1)),listefrequences,'.',color='black')
    if s1 == 1:
